...
- Step6 编码->代码验收
    - 任务描述：将`tasks/`中的任务列表，按照依赖顺序逐一完成编码和代码评审，**串行循环**执行每个任务，直到完成所有任务。执行过程如下
        - Phase1：[工程化编码]
        ...
        - Phase3：AI审查
            - 任务描述：审查`reviews/`目录下对应任务的`review-task-{任务编号}.md`文档，决策审查是否通过。可选的审查决策
                - 通过：进入下一个任务的执行，更新`state.json`记录任务执行状态
                - 不通过：
                    - 在`reviews/review-report.md`文档中记录审查、循环修复过程
                    - 返回Phase1 修复审查文档中的`Must Fix`项
                    - 每个任务的修复循环最多2轮
            - 执行角色：项目经理
